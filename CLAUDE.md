...
**Self-test:** `node .claude/hooks/__selftest.mjs` — guard hook'larının davranışını doğrulayan 122 assertion. Regresyon kapısının ilk komutu; kırmızıysa faz kırmızıdır. Yasak literal'leri (`'drop'+'db'` gibi) parça birleştirmeyle kurar, çünkü aksi hâlde test dosyasının kendisi guard'lara takılır.
